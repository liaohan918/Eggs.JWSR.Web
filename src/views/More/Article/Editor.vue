          ...
          -->
					<el-form-item label="详细" prop="bcontent">
						<div class="edit_container">
							<quill-editor v-model="infoForm.bcontent" ref="myQuillEditor" class="editer" :options="editorOption" @ready="onEditorReady($event)"></quill-editor>
						</div>
					</el-form-item>
...
<script>
	...
	import {
		quillRedefine
	} from "vue-quill-editor-upload"; // 上传图片
					...
					if (valid) {
						...
						this.$api.post("Blog", postPara, r => {
							if (r.success) {
								...
								this.$router.replace(`/More/ArticleDetail/${id}`);
							} else {
								var errorMsg = r.msg;
								this.$message({
									type: "error",
